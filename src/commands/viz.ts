...
export const vizCommand = new Command()
  ...
  .option('--mode <mode>', 'Visualization mode (tree, graph, list, interactive)', 'tree')
  .option('--depth <n>', 'Limit tree depth', parseInt)
  .option('--exclude <patterns>', 'Exclude patterns (comma-separated)')
  .option('--filter <patterns>', 'Include only patterns (comma-separated)')
  .option('--color', 'Enable colored output')
  .option('--export <format>', 'Export format (text, json, yaml)')
  .option('--with-metadata', 'Include metadata (size, modified time) in output')
  .action((path, options) => {
    try {
